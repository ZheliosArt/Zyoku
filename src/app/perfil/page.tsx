...
import CollectionsGrid from '@/components/CollectionsGrid' // <--- USA EL GLOBAL CON @
...
export default function Perfil() {
  ...
  const [tipoText, setTipoText] = useState('fan')
  ...
  useEffect(() => {
    const cargar = async () => {
      ...
      if (p) {
        ...
        setCommissionsOpen(p.commissions_open || false)
      ...
      // --- MEJORAMOS LA CONSULTA DE COLECCIONES ---
      const { data: cols, error: colError } = await supabase
        .from('colecciones')
        .select(`
          *,
          obras_guardadas(count),
          portadas:obras_guardadas(obras(imagen_url))
        `)
        .eq('usuario_id', userData.id)
        .order('created_at', { ascending: false });

      if (colError) console.error("Error en colecciones:", colError.message);
      ...
      // Conteo total de obras guardadas entre todas las carpetas
      const totalGuardados = coleccionesFormateadas.reduce((acc, c) => acc + (c.obras_guardadas?.[0]?.count || 0), 0);
      setGuardadosCount(totalGuardados)
  ...
  const abrirColeccion = async (col: any) => {
    ...
    if (error) {
      toast('Error al cargar la carpeta', 'err')
    } else {
      // Extraemos las obras y filtramos nulos
      const obrasExtraidas = data.map((d: any) => d.obras).filter(Boolean)
      setObrasDeColeccion(obrasExtraidas)
  ...
  const goToNext = () => {
    if (!obraZoom || !obrasActivas.length) return
