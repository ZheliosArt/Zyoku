...
// Nuevo estado para saber en qué colecciones está la obra actual
...
// 1. Creamos la colección real en Supabase
...
if (colError) {
toast('Error al crear la colección', 'err');
return;
}
...
if (saveError) {
toast('Error al guardar en la nueva colección', 'err');
// Si falla al guardar la obra, al menos mostramos la carpeta creada
const colFormateada = { ...nuevaCol, obras_guardadas: [{ count: 0 }] };
setColecciones(prev => [...prev, colFormateada]);
} else {
// SUCESO EXITOSO TOTAL
toast(`✓ Colección "${nombre}" creada y obra guardada`);
// 3. ACTUALIZAMOS EL ESTADO LOCAL TOTAL
...
savedCollections={savedCollections} // NUEVA PROP
onSave={handleSave}
