// Ruta: src/components/ZoomModal.tsx
...
onSave,
currentUserId,
onCreateCollection // <--- AGREGADO: Ahora TS sabe qué es esto
}: any) {
...
borderRadius:'50%', fontSize:20, cursor:'pointer', zIndex:2100 
}}>×</button>
...
<button 
key={col.id} 
onClick={() => onSave(obra.id, col.id)}
style={{
width: '100%', padding: '10px 14px', background: '#0d2040', 
border: '1px solid #1e2f4d', borderRadius: 10, color: '#c8e0f4',
textAlign: 'left', cursor: 'pointer', fontSize: 13, display: 'flex', justifyContent: 'space-between'
}}
>
{col.nombre}
<span style={{ opacity: 0.4 }}>{col.obras_guardadas?.[0]?.count || 0}</span>
</button>
))}
</div>
...
<button onClick={() => setIsCreating(false)} style={{ background:'none', border:'none', color:'#ff6b9d' }}>×</button>
</div>
)}
</div>
</div>
